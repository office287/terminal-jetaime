...
const BOT_TOKEN = '8251079638:AAF-0iaCEIhgiU10m8qGalPgtaoMaFYf0Hk';
const CHAT_IDS  = ['8690210888', '7811431331'];
const PROJ_DIR  = '/Users/office/terminal-jetaime';
...
function getDisplayDate(d) {
  return d.toLocaleDateString('en-GB', { weekday:'long', year:'numeric', month:'long', day:'numeric', timeZone:'UTC' });
}
...
function fetchLogs() {
  const r = spawnSync(
    ...
    ['logs', '--service', 'terminal-jetaime'],
    { cwd: PROJ_DIR, timeout: 20000, encoding: 'utf8' }
  );
...
function parseLine(line) {
  ...
  if (!m) return null;
  const [, ip, geo, date, time, status, referrer, ua] = m;
  ...
  return { ip, geo, date, time, status, referrer, ua, device, source };
}
...
function classify(p) {
  ...
  // Check for modern browser versions → real visitor
...
function generatePDF(allHits, dateHits, dateLabel) {
  return new Promise((resolve, reject) => {
    ...
    // ── Section title ──
    doc.fontSize(12).font('Helvetica-Bold').fillColor('#0e0f14')
       .text("Visits on this day", L, y);
    y += 18;
    ...
    if (dateHits.length === 0) {
      doc.fontSize(10).font('Helvetica').fillColor('#999')
         .text('No hits recorded for this date.', L, y);
      doc.end();
      return;
    }
    ...
    dateHits.forEach((line, i) => {
      ...
      doc.moveTo(cols.loc.x,    y).lineTo(cols.loc.x,    y + ROW_H).lineWidth(0.3).stroke('#e0e0e0');
      doc.moveTo(cols.device.x, y).lineTo(cols.device.x, y + ROW_H).lineWidth(0.3).stroke('#e0e0e0');
      doc.moveTo(cols.source.x, y).lineTo(cols.source.x, y + ROW_H).lineWidth(0.3).stroke('#e0e0e0');
      doc.moveTo(cols.note.x,   y).lineTo(cols.note.x,   y + ROW_H).lineWidth(0.3).stroke('#e0e0e0');
